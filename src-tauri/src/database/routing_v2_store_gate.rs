...
fn store_state_table_exists(conn: &Connection) -> Result<bool, StoreGateError> {
    // SQLite 标识符按 ASCII 大小写无关匹配；`=` 比较对文本是大小写敏感的，
    // 因此先取出全部表名再在 Rust 侧做 eq_ignore_ascii_case 精确比较。
    // 不使用 LIKE 前缀匹配，避免下划线被当作通配符。
    ...
    let mut rows = statement
        .query([])
        .map_err(|_| StoreGateError::Unavailable)?;
    while let Some(row) = rows.next().map_err(|_| StoreGateError::Unavailable)? {
        let name: String = row.get(0).map_err(|_| StoreGateError::Unavailable)?;
        if name.eq_ignore_ascii_case(STORE_STATE_TABLE) {
            return Ok(true);
        }
    }
    Ok(false)
    ...
    let mut statement = conn
        .prepare(
            "SELECT id, migration_epoch, minimum_reader_version, rollback_generation \
             FROM main.routing_v2_store_state ORDER BY id LIMIT 2",
        )
        .map_err(|_| StoreGateError::Unavailable)?;
    let mut rows = statement
        .query([])
...
mod tests {
    ...
    #[test]
    fn gate_accepts_state_table_with_differing_ascii_case() {
        let conn = Connection::open_in_memory().expect("打开内存数据库");
        conn.execute_batch(
            "CREATE TABLE ROUTING_V2_STORE_STATE (\
                id INTEGER,\
    ...
    fn create_temp_state_table(conn: &Connection) {
        conn.execute_batch(
    ...
    #[test]
    fn gate_temp_valid_state_cannot_override_invalid_main_state() {
        let conn = state_only_connection();
        insert_state(&conn, 1, 0, 1, 0);
        create_temp_state_table(&conn);

        assert!(matches!(acquire(&conn), Err(StoreGateError::StateInvalid)));
    }

    #[test]
    fn gate_temp_only_state_cannot_satisfy_main_presence() {
        let conn = Connection::open_in_memory().expect("打开内存数据库");
        create_temp_state_table(&conn);
