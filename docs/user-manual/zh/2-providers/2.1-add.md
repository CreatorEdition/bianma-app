...
通过 `bianma://` 协议链接一键导入；如需查看旧 scheme 与旧命名的兼容范围，请见[迁移兼容说明](../5-faq/5.5-migration-from-cc-switch.md)。
...
> ⚠️ **注意**：导入会覆盖现有数据库，建议先导出当前配置作为备份。当前导出的文件名可能仍沿用历史前缀 `cc-switch-export-{时间戳}.sql`，该前缀属于兼容标识。
