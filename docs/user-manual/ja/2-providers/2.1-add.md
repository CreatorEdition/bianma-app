...
`bianma://` プロトコルリンクでワンクリックインポートできます。旧 scheme や旧命名の互換範囲は [移行互換ガイド（ZH）](../../zh/5-faq/5.5-migration-from-cc-switch.md) を参照してください。
...
> **注意**：インポートは既存のデータベースを上書きするため、事前に現在の設定をエクスポートしてバックアップすることをお勧めします。エクスポートファイルは互換性のため、歴史的な `cc-switch-export-{タイムスタンプ}.sql` プレフィックスを引き続き使用する場合があります。
