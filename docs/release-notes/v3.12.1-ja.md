...
> 安定性修正、StepFun プリセット、OpenClaw authHeader 対応、スポンサーパートナー更新
...
bianma-app v3.12.1 は、安定性の改善とバグ修正に焦点を当てたパッチリリースです。共通設定モーダルの無限再オープンループ、WebDAV 同期時の外部キー制約エラー、複数の i18n 補間問題を修正しました。また、**StepFun（阶跃星辰）** プロバイダープリセットの追加、OpenClaw の**入力タイプ選択**と **authHeader** サポート、デフォルト Gemini モデルの **3.1-pro** へのアップグレード、4 件のスポンサーパートナー更新が含まれます。

**リリース日**: 2026-03-12

**更新規模**: 19 commits | 56 files changed | +1,429 / -396 lines

> 歴史的な注記: このリリースで記録しているスポンサー、そして下で紹介する CC Switch ブランドの名前を使ったダウンロード情報は、当時の配布状況をそのまま残すアーカイブ記録です。新たな配布促進を意図したものではありません。
...
- **スポンサーパートナー更新**: Micu API、XCodeAPI、SiliconFlow、CTok を履歴上のスポンサーパートナー記録に追加
...
- **Micu API**: Micu API をスポンサーパートナーとして追加
- **XCodeAPI**: XCodeAPI をスポンサーパートナーとして追加
- **SiliconFlow**: SiliconFlow（硅基流动）をスポンサーパートナーとして追加
...
[Releases](https://github.com/CreatorEdition/bianma-app/releases/latest) から適切なバージョンをダウンロードしてください。
