...
あるアプリのスイッチをオンにすると、bianma-app は以下を実行します：
...
あるアプリのスイッチをオフにすると、bianma-app は以下を実行します：
...
CLI ツールで既に MCP サーバーを設定している場合、bianma-app にインポートできます：

1. 「インポート」ボタンをクリック
2. インポートするアプリを選択（Claude/Codex/Gemini/OpenCode）
3. bianma-app が既存の設定を読み取ってインポート
