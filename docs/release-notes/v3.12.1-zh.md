...
> 稳定性修复、StepFun 预设、OpenClaw authHeader 支持，以及赞助伙伴更新
...
bianma-app v3.12.1 是一个以稳定性改进和 Bug 修复为主的补丁版本。修复了通用配置弹窗无限重复打开的循环问题、WebDAV 同步时的外键约束失败以及多个 i18n 插值问题。同时新增了 **StepFun（阶跃星辰）** 供应商预设、OpenClaw **输入类型选择** 和 **authHeader** 支持，将默认 Gemini 模型升级到 **3.1-pro**，并记录了四项赞助伙伴更新。

**发布日期**：2026-03-12

**更新规模**：19 commits | 56 files changed | +1,429 / -396 lines

> 历史说明：下文的赞助描述与带 CC Switch 名称的下载记录还原了当时的发布配布，仅作存档参考，不作为当前推广。
...
- **赞助伙伴（归档）**：记录 Micu API、XCodeAPI、SiliconFlow、CTok 作为赞助伙伴的历史备注。
...
> 历史说明：下表的安装包名称和安装命令保留了当时的 CC Switch 命名，仅作为存档参考，而非当前推广。

访问 [Releases](https://github.com/CreatorEdition/bianma-app/releases/latest) 下载对应版本。
...
| 文件                                       | 说明                                |
| ------------------------------------------ | ----------------------------------- |
| `CC-Switch-v3.12.1-Windows.msi`            | MSI 安装包，支持自动更新。 |
...
| 文件                               | 说明                                                      |
| ---------------------------------- | --------------------------------------------------------- |
| `CC-Switch-v3.12.1-macOS.zip`      | ZIP 包，拖入 Applications（Universal Binary）。 |
...
> 历史说明：下列命令指向遗留的 `cc-switch` Homebrew tap，仅作存档参考。
...
> 历史说明：Linux 表格中的包名也保留了 CC Switch 前缀，作为归档记录。
