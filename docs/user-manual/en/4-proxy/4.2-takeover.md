...
App takeover means letting bianma-app's proxy intercept and forward a specific application's API requests.
...
When takeover is enabled, bianma-app modifies the app's configuration file to point the API endpoint to the local proxy.
...
When disabling takeover, bianma-app will:
...
Before enabling takeover, bianma-app backs up the original configuration and restores it when disabled.
