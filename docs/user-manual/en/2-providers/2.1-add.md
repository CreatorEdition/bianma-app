...
One-click import via `bianma://` protocol links. For legacy scheme and naming compatibility details, see the [migration compatibility guide (ZH)](../../zh/5-faq/5.5-migration-from-cc-switch.md).
...
> **Note**: Importing will overwrite the existing database. It is recommended to export your current configuration as a backup first. Exported files may still use the historical `cc-switch-export-{timestamp}.sql` prefix as a compatibility identifier.
