...
//!
//! Parses deep-link URLs into DeepLinkImportRequest structures.
...
///
/// Expected format:
/// bianma://v1/import?resource={type}&...
pub fn parse_deeplink_url(url_str: &str) -> Result<DeepLinkImportRequest, AppError> {
    ...
    if !matches!(scheme, "bianma" | "ccswitch") {
        return Err(AppError::InvalidInput(format!(
            "Invalid scheme: expected 'bianma' or legacy 'ccswitch', got '{scheme}'"
        )));
    }
